    ...
    <div className="w-full bg-[#FAFAFA]">
      ...
      <section className="py-20 sm:py-24 px-6 bg-white border-t border-b border-[#E2E8F0] relative overflow-hidden">
        <div className="w-full max-w-[1920px] mx-auto lg:px-12 xl:px-20">
          <div className="text-center max-w-3xl mx-auto mb-16 sm:mb-20">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-serif font-bold text-[#0B1221] mb-4">Why Partner With SEQ Services</h2>
            <p className="text-[#334155] text-lg sm:text-xl md:text-2xl font-medium">Built on compliance, safety, operational integrity, and exceptional standards.</p>
          </div>
            ...
            {ABOUT_DATA.values.map((val, idx) => {
              ...
              return (
                <motion.div 
                  ...
                  className="bg-[#F8FAFC] p-8 rounded-3xl border border-[#E2E8F0] hover:border-[#29B6F6]/40 hover:shadow-xl transition-all duration-300 group"
                >
                  <div className="w-16 h-16 rounded-2xl bg-[#29B6F6]/10 text-[#29B6F6] flex items-center justify-center mb-6 group-hover:bg-[#29B6F6] group-hover:text-white transition-colors duration-300">
                    <Icon size={28} />
                  </div>
                  <h3 className="text-2xl sm:text-3xl font-serif font-bold text-[#0B1221] mb-4">{val.title}</h3>
                  <ul className="space-y-3">
                    {val.points.map((point, pIdx) => (
                      <li key={pIdx} className="text-[#334155] text-base sm:text-lg leading-relaxed font-semibold flex items-center gap-2.5">
      ...
      <section className="py-20 sm:py-24 px-6 bg-[#F8FAFC] border-b border-[#E2E8F0] relative overflow-hidden">
        <div className="w-full max-w-[1920px] mx-auto lg:px-12 xl:px-20">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="text-sm font-extrabold uppercase tracking-widest text-[#29B6F6] bg-[#29B6F6]/10 px-5 py-2 rounded-full border border-[#29B6F6]/20 inline-block mb-4 shadow-sm">
              Certified & Compliant
            </span>
            <h2 className="text-3xl sm:text-4xl md:text-6xl font-serif font-bold text-[#0B1221] mb-4">Our Industry Certifications</h2>
            <p className="text-[#334155] text-lg sm:text-xl md:text-2xl font-medium">Accredited to international standards for safety, quality management, and environmental compliance.</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div
                key={cIdx}
                variants={fadeUpVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}
                className="bg-white p-8 md:p-10 rounded-3xl border border-[#E2E8F0] shadow-sm hover:shadow-xl hover:border-[#29B6F6]/40 transition-all duration-300 flex flex-col justify-between group relative overflow-hidden"
              >
                <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-[#29B6F6] to-[#00a8cc]" />
                
                <div>
                  <div className="h-32 w-full rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center p-4 mb-6 group-hover:scale-105 transition-transform shadow-inner">
                    <img 
                      src={cert.image} 
                      alt={cert.title} 
                  ...
                  <span className="text-sm font-extrabold uppercase tracking-widest text-[#29B6F6] bg-[#29B6F6]/10 px-4 py-1.5 rounded-lg border border-[#29B6F6]/20 inline-block mb-3">
                    {cert.code}
                  </span>
                  <h3 className="text-2xl sm:text-3xl font-serif font-bold text-[#0B1221] mb-4">{cert.title}</h3>
                  <p className="text-[#334155] text-base sm:text-lg leading-relaxed font-medium">{cert.desc}</p>
                </div>
      ...
      <section className="py-20 px-6 bg-fluid-wave-fixed text-center relative overflow-hidden">
        <div className="max-w-4xl mx-auto relative z-10">
          <h2 className="text-3xl md:text-5xl font-serif font-bold text-[#0B1221] mb-6">Ready to elevate your commercial facility management?</h2>
          <p className="text-lg sm:text-xl md:text-2xl text-[#334155] mb-10 font-normal max-w-2xl mx-auto">Get in touch with our expert team today for a tailored operational proposal and quote.</p>
          <Link href="/#quote" className="inline-block px-10 py-4.5 bg-[#29B6F6] text-white text-lg sm:text-xl font-bold rounded-2xl shadow-lg hover:bg-[#0042a3] transition-all">
